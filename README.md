...
Main Script: The primary file follows the logical flow of the analysis. Each major computational step—solving for \tilde \xi and \tilde Q_v^0, determining the capillary limit, calculating p* vs z*, etc. Sections are explicitly named at their beginning.
...
Set Fluid Properties & Solver Parameters: In the first section, configure settings such as temperature (e.g., 60, 200, or 300). Then define the Geometry inn the second section, specify geometric parameters (e.g., set Rg_star to 1.18, 1.20, or 1.22 mm).

Execute Sections Sequentially: Follow the in-line comments and run the code section-by-section in the provided order. Most sections depend on outputs from previous ones (e.g., the capillary limit calculation requires the solutions for \tilde \xi and \tilde Q_v).
