...
nexttile
    ...
    p2 = plot([0],ResultAnaly_CB,'+',LineWidth=1.5,DisplayName='B',Color='#EDB120')
    % plot([0],ResultNumcl_CB,'o',LineWidth=1.5,DisplayName='B numerical',Color='#EDB120')
    p3 = plot(tm0,ResultNumrical(1,:),'.',DisplayName='C',Color='#0072BD');
    ...
    xlim([-0.2 1.8]);  ylim([-30 -5]); % ylim([-10^(3) 0]); %
...
load('XiRgs1.22e+00-1.22e+00_T60_2024-12-28.mat');
    ResultNumrical = tXi0N;
    ResultAnalytic = tXi0A(:,1); %+ tXi0A(:,2).*tm0;
    semilogy(tm0,ResultNumrical(1,:),'.',DisplayName='$\tilde{\xi}~[\tilde\rho(60^{\circ}\rm{C}),\tilde\mu(60^{\circ}\rm{C})]$',Color='#D95319');hold on;
    load('XiRgs1.22e+00-1.22e+00_T60_Tmu200_2025-03-15');
    ResultNumrical = tXi0N;
    semilogy(tm0,ResultNumrical(1,:),'.',DisplayName='$\tilde{\xi}~[\tilde\rho(60^{\circ}\rm{C}),\tilde\mu(200^{\circ}\rm{C})]$',Color='#0072BD');
    ...
    ResultNumrical = tXi0N;
    ResultAnalytic = tXi0A(:,1); %+ tXi0A(:,2).*tm0;
    semilogy(tm0,ResultNumrical(1,:),'.',DisplayName='$\tilde{\xi}~[\tilde\rho(200^{\circ}\rm{C}),\tilde\mu(60^{\circ}\rm{C})]$',Color='#EDB120')
    % semilogy([0],ResultAnalytic(1,:),'*',DisplayName='$\tilde{\xi}^0~[\tilde\rho(200^{\circ}\rm{C}),\tilde\mu(60^{\circ}\rm{C})]$',Color='#4DBEEE')
    load('XiRgs1.22e+00-1.22e+00_T200_2024-12-28.mat');
    ResultNumrical = tXi0N;
    semilogy(tm0,ResultNumrical(1,:),'.',DisplayName='$\tilde{\xi}~[\tilde\rho(200^{\circ}\rm{C}),\tilde\mu(200^{\circ}\rm{C})]$',Color='#77AC30')
    ylabel('$\tilde{\xi}$',Rotation=0,FontSize=fsAxi)
    xlim([-0.2 2.1]); ylim([-10^(3) -1]); %
    xticks([-0.2 0 0.2 0.4 0.6 0.8 1 1.2 1.4 pi/2])
    xticklabels({'-0.2' '0' '0.2' '0.4' '0.6' '0.8' '1' '1.2' '1.4' '\pi/2'})
...
for i_row = 1:2
    ...
    plot(z_starN1,p_l_star_CaseA*1E-6,'--',Color='#77AC30',LineWidth=ls,HandleVisibility='off');
    plot(z_starN1,p_v_star_CaseB*1E-6,'-.',Color='#EDB120',LineWidth=ls,DisplayName='B');
    plot(z_starN1,p_l_star_CaseB*1E-6,'-.',Color='#EDB120',LineWidth=ls,HandleVisibility='off')
    lgd = legend(Location='best',Orientation='horizontal',Box='off',FontSize=fsLgd)    
    title(lgd,titleCell{i_colum,i_row})
    titleCell{i_colum,i_row}
...
CArr = ["#0072BD","#D95319","#77AC30","#EDB120"];
...
hold on; box on
    ...
    nameStr = '$q^{*^{\rm{C0}}}_{\mathrm{cap}}$';
    plot(phi,qc0A_C0_g,'-' ,LineWidth = lw,Color=CArr(2),DisplayName=nameStr)
    nameStr = '$q^{*^{\rm{A}}}_{\mathrm{cap}}$';
    plot(phi,qc_CaseA_g,'--' ,LineWidth = lw,Color=CArr(3),DisplayName=nameStr)
    nameStr = '$q^{*^{\rm{B}}}_{\mathrm{cap}}$';
    plot(phi,qc_CaseB_g,'-.' ,LineWidth = lw,Color=CArr(4),DisplayName=nameStr)
    xlim([-pi/6  pi*7/6])
    xticks([-pi/6 0 pi/6 pi/3 pi/2 pi*2/3 pi*5/6 pi pi*7/6])
    xticklabels({'-\pi/6' '0' '\pi/6' '\pi/3' '\pi/2' '2\pi/3' '5\pi/6' '\pi' '7\pi/6'})
...
plot(z_star  ,p_v_starA*0.001,'-',Color='#D95319'    ,LineWidth=1,DisplayName='$p^{*^{0}}$');
plot(z_star  ,p_l_starA*0.001,'-',Color='#D95319'    ,LineWidth=1,HandleVisibility='off');
plot(z_star,p_v_star_CaseA*0.001,'--',Color='#77AC30',MarkerSize=3,DisplayName='$p^{*^{\rm{A}}}$');
plot(z_star,p_l_star_CaseA*0.001,'--',Color='#77AC30',MarkerSize=3,HandleVisibility='off');
plot(z_star,p_v_star_CaseB*0.001,'-.',Color='#EDB120',MarkerSize=3,DisplayName='$p^{*^{\rm{B}}}$');
plot(z_star,p_l_star_CaseB*0.001,'-.',Color='#EDB120',MarkerSize=3,HandleVisibility='off');
% xline(0,'-',{'Adiabatic','begin'},LabelVerticalAlignment='bottom');
% xline(La_star(iz),HandleVisibility='off')
xlabel('$z^*$ (m)'); ylabel('$p^*$ (kPa)'); 
xlim([-0.1 0.2])
...
hfigname = ['Effects_Gravity\p_star_vs_z_star_v2' sprintf('_T%i',T) '_Rgstr' num2str(Rg_star(ig)*100000) '_phi157'];
