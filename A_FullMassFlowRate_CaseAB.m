...
%% Step 9-AB solveing for \tilde \xi for case A and B
% below copied from Matlab_Mathematica\FinalPlots\PressurePlots.m:
Fn_star = 4*(-1).^n./(R_l_star.^(gamma-1) + ...
...
% CASE B ISSUE test 7/16/2025
% K4_star_terms2 = 4*(-1).^(2.*n).*(R_l_star.^2*R.^(2*gamma).*(2-gamma)+2*R_g_star.*gamma.*R.^gamma...
%     -R_l_star.^2.*(2+gamma))./(t_g.*gamma.^3.*(gamma.^2-4).*...
%     (R_l_star.^(gamma-1)+R_g_star.^(2*gamma).*R_l_star.^(-gamma-1)));

% K4_star_terms2 = (4*(-1).^(2*n)./(t_g.*gamma.^3)) .* (R_l_star./(R.^(2*gamma)+1))...
%     .*( ...
%      (R_g_star.^2.*R.^gamma-R_l_star.^2.*R.^(2*gamma))./(2+gamma) ...
%     +(R_l_star.^2 -R.^gamma.*R_g_star.^2)./(2-gamma)...
%     ); 

K4_star_terms2 = 4*(-1).^(2*n)./(t_g.*gamma.^3.*(4-gamma.^2))...
    .* (R_l_star./(R.^(2*gamma)+1))...
    .*( ...
     (2-gamma).*(R_g_star.^2.*R.^gamma - R_l_star.^2.*R.^(2*gamma))...
    +(2+gamma).*(R_l_star.^2           - R_g_star.^2.*R.^gamma)...
    ); 

K4_star_terms3 = 4*(-1).^(2*n)./(t_g.*gamma.^3.*(4-gamma.^2)) ...
    .* (R_l_star.^3)...
    .*( (gamma-2).*R.^(2*gamma) + 2 + gamma)./(R.^(2*gamma)+1) ; 


K4_star = sum(K4_star_terms2);
